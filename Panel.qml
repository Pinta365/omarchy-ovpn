...
Panel {
  ...
  function moveCursor(dx, dy) {
    var step = dy !== 0 ? dy : dx
    cursor = Math.max(0, Math.min(stops.length - 1, cursor + step))
  ...
  KeyboardPanel {
    ...
    PanelKeyCatcher {
      ...
      onActivateRequested: {
        if (!root.cursorActive) { vpn.quickToggle(); return }
        root.activateCursor()
      }
      onCloseRequested: {
      ...
      Flickable {
        ...
        Column {
          ...
          Column {
            ...
            PanelSectionHeader {
              width: parent.width
              text: vpn.authFailed ? "Wrong username or password" : "Sign in to OVPN"
